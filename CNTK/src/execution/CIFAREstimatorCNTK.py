    ...
    training_session(
        ...
        checkpoint_config=CheckpointConfig(frequency=epoch_size,
                                           filename=os.path.join(
                                               model_path, "ConvNet_CIFAR10_DataAug"),
                                           restore=restore),
...
def convnet_cifar10_dataaug(train_data, test_data, mean_data, minibatch_size=64, epoch_size=50000, num_quantization_bits=32,
    ...
    trainer = create_trainer(
        network, epoch_size, num_quantization_bits, block_size, warm_up, progress_writers)
    ...
    train_and_test(network, trainer, train_source, test_source,
                   minibatch_size, epoch_size, restore, profiling, model_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-o', '--outputdir', help='Output directory for checkpoints and models', required=False, default=None)
    parser.add_argument('-l', '--logdir', help='Log file',
                        required=False, default=None)
    parser.add_argument('-t', '--tensorboard_logdir', help='Directory where TensorBoard logs should be created',
                        required=False, default=None)
    parser.add_argument('-n', '--num_epochs', help='Total number of epochs to train',
                        type=int, required=False, default='160')
    parser.add_argument('-m', '--minibatch_size', help='Minibatch size',
                        type=int, required=False, default='64')
    parser.add_argument('-e', '--epoch_size', help='Epoch size',
                        type=int, required=False, default='50000')
